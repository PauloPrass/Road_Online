class BusLinesController < ApplicationController
  ...
  def index
    ...
    if filter != nil
      @bus_lines = BusLine.where("name LIKE ?", "%#{filter}%")
    else
  ...
  def filter_lines
    ...
    @bus_lines = BusLine.where("name LIKE ?", "%#{filter}%")
    respond_to do |format|
      format.html { redirect_to bus_lines_url, notice: "Bus line was successfully filtered." }
  ...
  private
    ...
    def bus_line_params
      params.require(:bus_line).permit(:code, :name)
    end
end
